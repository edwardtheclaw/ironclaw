...
async fn test_resolve_metadata_no_config() {
    ...
    assert_eq!(
        meta.skip_indexing, None,
        "skip_indexing should be None by default"
    );
    assert_eq!(
        meta.skip_versioning, None,
        "skip_versioning should be None by default"
    );
...
async fn test_write_creates_version() {
    ...
    let v1 = ws
        .get_version(doc.id, versions[0].version)
        .await
        .expect("get v1");
    assert_eq!(
        v1.content, "v1",
        "version content should be the original 'v1'"
    );
}
...
async fn test_write_deduplicates_identical_content() {
    ...
    ws.write("file.md", "same")
        .await
        .expect("write second (identical)");
...
async fn test_append_versions_pre_append_content() {
    ...
    assert_eq!(
        versions.len(),
        1,
        "append should create 1 version of pre-append content"
    );
    ...
    assert_eq!(
        v1.content, "line1",
        "version should capture pre-append content"
    );
}
...
async fn test_patch_not_found_error() {
    ...
    assert!(
        err_str.contains("not found")
            || err_str.contains("PatchFailed")
            || err_str.contains("patch"),
...
async fn test_skip_indexing_via_config() {
    ...
    ws.write(
        "frontend/widget.js",
        "function render() { return <div>Hello</div>; }",
    )
    .await
    .expect("write widget");
