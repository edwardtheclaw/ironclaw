...
def compute_cone(current_rate, daily_vol, vb, today_day):
    ...
    cone_upper = current_rate * math.exp(CONE_Z * daily_vol * math.sqrt(HORIZON_DAYS))
    recommend = "now" if target_rate > cone_upper else "wait"
    projection = []
    ...
    return target_rate, recommend, projection
...
target_rate, recommend, projection = compute_cone(current_rate, daily_vol, vb, today_day)
