                ...
                let perms = if let Some(store) = self.tenant.store() {
                    match store.get_all_settings().await {
                        ...
                        Err(e) => {
                            ...
                            // Don't clear session approvals — keep whatever was set before.
                            // Tool definitions are still refreshed (above), only permission
                            // filtering/auto-approve is skipped this iteration.
