...
| `static/init.js` | Early initialization (theme FOUC prevention, debug mode detection) — runs before `app.js` |
| `static/debug-panel.js` | Debug inspector panel (IIFE, only activates when `?debug=true`) |
| `static/debug-panel.css` | Debug panel styling (sidebar, tabs, activity timeline, stats) |
...
| GET | `/api/debug/prompt` | Debug: system prompt components, model, context limit |
...
| GET | `/init.js` | Early initialization (theme + debug mode) |
| GET | `/debug-panel.js` | Debug inspector panel JS |
| GET | `/debug-panel.css` | Debug inspector panel CSS |
...
| `image_generated` | Image generated by tool |
| `suggestions` | Suggested follow-up messages |
| `turn_cost` | Per-turn token usage and cost |
| `reasoning_update` | Agent's tool decision rationale |
| `job_reasoning` | Reasoning for sandbox job |
| `tool_result_full` | **Verbose only** — full tool output (up to 50KB, scrubbed) |
| `turn_metrics` | **Verbose only** — per-LLM-call model, tokens, cache, duration |

### Verbose SSE Mode (`?debug=true`)

The SSE endpoint `/api/chat/events` accepts an optional `?debug=true` query parameter. When set, the subscriber receives **all** events including verbose-only types (`tool_result_full`, `turn_metrics`). Non-verbose subscribers (the default) never see these events.

Filtering is implemented in `SseManager::subscribe()` via `AppEvent::is_verbose_only()`. The broadcast channel sends all events to all subscribers; filtering happens per-subscriber in the stream pipeline.
...
A right-sidebar debug panel activated via `?debug=true` URL parameter. Implemented entirely in separate files (`debug-panel.js`, `debug-panel.css`) to avoid merge conflicts with `app.js`/`style.css`.

**Activation:** `?debug=true` → sets `window.isDebugMode` in `init.js` → persisted to `sessionStorage`. The `<>` toolbar button toggles the panel open/closed.

**Architecture:**
- `init.js` runs in `<head>` before `app.js` — sets `window.isDebugMode`
- `app.js` `connectSSE()` checks `window.isDebugMode` to append `&debug=true` to SSE URL, calls `window.onDebugSSEConnect(eventSource)` hook
- `debug-panel.js` registers the hook to attach event listeners

**Three tabs:**
1. **Prompt** — fetches `/api/debug/prompt`, shows component breakdown with token counts, full system prompt in collapsible `<pre>`, model name + context progress bar
2. **Activity** — real-time SSE event timeline per turn. Events stored with `turn` number, navigable via prev/next buttons. Clicking a chat message switches to that turn. Capped at 1000 entries.
3. **Stats** — session counters (turns, tokens, cost, tool calls), per-model usage from `/api/gateway/status`, SSE health

**Responsive:** 380px sidebar on desktop, overlay on <1024px, hidden on <768px.

**XSS prevention:** All tool output rendered via `textContent`, never `innerHTML` (except hardcoded SVG icon).
