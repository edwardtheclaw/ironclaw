...
mod tests {
    ...
    async fn test_restart_with_addr_rollback_on_bind_failure() {
        ...
        // Try to restart on the same address the server already holds, which
        // should fail because the port is in use (works even when running as root,
        // unlike binding to a privileged port).
        ...
        assert!(result.is_err(), "Bind to already-in-use port should fail");
